...
export class Visual implements IVisual {
    ...
    private render(options: VisualUpdateOptions): void {
        ...
        g.append("g").call(
            ...
            sel.selectAll("line").attr("stroke", settings.gridColor).attr("stroke-dasharray", "2,3").attr("opacity", 0.5);
        });
        ...
        g.append("g").attr("transform", `translate(0,${plotH})`)
            .call(d3.axisBottom(xScale).tickFormat(d => fmtNum(+d)).ticks(Math.min(effectiveBins, 8)))
            .call(sel => {
                sel.select(".domain").attr("stroke", settings.axisColor);
                sel.selectAll("text").attr("fill", settings.axisColor).attr("font-size", settings.axisFontSize);
        ...
        if (settings.showYLabel) {
            g.append("text").attr("transform", "rotate(-90)").attr("x", -plotH / 2).attr("y", -marginL + 12)
                .attr("text-anchor", "middle").attr("font-size", settings.axisFontSize).attr("fill", settings.axisColor).text("Count");
        }
        if (settings.showXLabel) {
            g.append("text").attr("x", plotW / 2).attr("y", plotH + marginB - 6)
                .attr("text-anchor", "middle").attr("font-size", settings.axisFontSize).attr("fill", settings.axisColor).text("Value");
        }
        ...
        if (settings.showMean && mean >= xDomain[0] && mean <= xDomain[1]) {
            ...
            g.append("line").attr("x1", mx).attr("y1", 0).attr("x2", mx).attr("y2", plotH)
                .attr("stroke", settings.meanColor).attr("stroke-width", 1.5).attr("stroke-dasharray", "4,3");
            g.append("text").attr("x", mx + 4).attr("y", 10)
                .attr("fill", settings.meanColor).attr("font-size", settings.axisFontSize).text(`μ ${fmtNum(mean)}`);
        }
        ...
        if (settings.showMedian && median >= xDomain[0] && median <= xDomain[1]) {
            ...
            g.append("line").attr("x1", mdx).attr("y1", 0).attr("x2", mdx).attr("y2", plotH)
                .attr("stroke", settings.medianColor).attr("stroke-width", 1.5).attr("stroke-dasharray", "6,3");
            g.append("text").attr("x", mdx + 4).attr("y", 24)
                .attr("fill", settings.medianColor).attr("font-size", settings.axisFontSize).text(`M ${fmtNum(median)}`);
        }
        ...
        if (this.isPro && settings.showStats) {
            const lines = [
                ...
                `μ = ${fmtNum(mean)}`, `M = ${fmtNum(median)}`, `σ = ${fmtNum(std)}`,
                `min = ${fmtNum(dMin)}`, `max = ${fmtNum(dMax)}`,
            ];
    ...
    public getFormattingModel(): powerbi.visuals.FormattingModel {
        ...
        const col = (uid: string, name: string, obj: string, prop: string, val: string) => ({
            uid, displayName: name,
            control: { type: powerbi.visuals.FormattingComponent.ColorPicker, properties: { descriptor: { objectName: obj, propertyName: prop }, value: { value: val } } }
        });
        ...
        return {
            cards: [
                card("histogram_card", "Histogram", [
                    ...
                    col("barColor",    lbl("Bar color"),      "histogram", "barColor",    s.barColor),
